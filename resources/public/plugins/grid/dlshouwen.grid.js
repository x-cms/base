                ...
                constructGrid : function(){
                    ...
                    if(gridReflectionObj.exhibitDatas!=null){
                        ...
                        for(var i=0; i<gridReflectionObj.exhibitDatas.length; i++){
                            gridContent += '	<tr class="grid-row " dataNo="'+i+'">';
                            if(gridReflectionObj.option.extraWidth!=null){
                                gridContent += '	<td class="extra-column extra-column-event '+extraColumnClass+'" width="'+gridReflectionObj.option.extraWidth+'" dataNo="'+i+'">'+$.fn.dtGrid.lang[gridReflectionObj.option.lang].extraColumn.open+'</td>';
                            }else{
                                gridContent += '	<td class="extra-column extra-column-event '+extraColumnClass+'" dataNo="'+i+'">'+$.fn.dtGrid.lang[gridReflectionObj.option.lang].extraColumn.open+'</td>';
                            }
                            if(gridReflectionObj.option.check){
                                if(gridReflectionObj.option.checkWidth!=null){
                                    gridContent += '	<td class="check-column text-center" width="'+gridReflectionObj.option.checkWidth+'"><input type="checkbox" dataNo="'+i+'" id="grid_'+gridReflectionObj.option.id+'_check_'+i+'" class="grid-check" value="'+i+'"></td>';
                                }else{
                                    gridContent += '	<td class="check-column text-center"><input type="checkbox" dataNo="'+i+'" id="grid_'+gridReflectionObj.option.id+'_check_'+i+'" class="grid-check" value="'+i+'"></td>';
                                }
                            }
                            var columns = gridReflectionObj.option.columns;
                            for(var j=0; j<columns.length; j++){
                                if(columns[j].width!=null){
                                    gridContent += '	<td width="'+columns[j].width+'" dataNo="'+i+'" columnNo="'+j+'" class="grid-cell text-center '+gridReflectionObj.getColumnClassForHide(columns[j])+' '+columns[j].columnClass+'" style="'+columns[j].columnStyle+'">';
                                }else{
                                    gridContent += '	<td dataNo="'+i+'" columnNo="'+j+'" class="grid-cell text-center '+gridReflectionObj.getColumnClassForHide(columns[j])+' '+columns[j].columnClass+'" style="'+columns[j].columnStyle+'">';
                                }
                            ...
                            gridContent += '	</tr>';
                            gridContent += '	<tr id="grid_'+gridReflectionObj.option.id+'_extra_columns_'+i+'" class="grid-extra-columns hidden">';
                            gridContent += '		<td dataNo="'+i+'" colspan="'+(columns.length+1+(gridReflectionObj.option.check?1:0))+'">';
                            for(var j=0; j<columns.length; j++){
                                if(columns[j].extra==false){
                                    continue;
                                }
                                gridContent += '		<p dataNo="'+i+'" columnNo="'+j+'" class="grid-cell '+gridReflectionObj.getExtraColumnClassForHide(columns[j])+'">';
